...
function put_nmonlog(req, res, bulk_unit) {
    ...
    var parser = new NmonParser({objectMode: true});
...
NmonParser.prototype._transform = function(data, encoding, done) {
    ...
    if( data[0].substring(0, 3) === 'AAA' ) {
        ...
        loggerParser.write('['+ this._hostname + ':AAA]\033[m ' + data[1] + ',' + data[2] );
        ...
        loggerParser.write('\n\033[1;34m[' + now.toLocaleTimeString() + ']-');
        loggerParser.write('['+ this._hostname + ':' + data[0] + ':' + data[1] + ']\033[m ' + data[2] + ',' + data[3]);
    }
        ...
        loggerParser.write('\n\033[1;34m[' + now.toLocaleTimeString() + ']-');
        loggerParser.write('['+ this._hostname + ':ZZZZ:' + data[1] + ']\033[m ');
        ...
        if( data[0] in this._rawHeader ) {
            ...
            if ((h[0] === 'TOP' || h[0] === 'UARG') && this._cntTU % 80 == 0) {
                loggerParser.write('\n\033[1;34m[' + now.toLocaleTimeString() + ']-');
                loggerParser.write('['+ this._hostname + ':ZZZZ:' + 
